...
print("Reset obs:",obs["address_validation_status"],obs["device_validation_status"],obs["sim_validation_status"])
for i in range(10):
    action, _state = model.predict(obs, deterministic=True)
    print("Taking action:", action)
    obs, reward, done, terminated, info = env.step(action)
    print("episod ", i, "obs:",obs["address_validation_status"],obs["device_validation_status"],obs["sim_validation_status"], "reward:", reward, done)
    #env.render()
    if done:
      obs,_ = env.reset()
